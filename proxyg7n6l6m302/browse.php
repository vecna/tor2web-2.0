...
switch ( true ) {
   ...
   case ! empty($_GET['antani']) && ( $toLoad = deproxifyURL($_GET['antani'], true) ):
      break;
...
while(isset($querystring[$qs_nos])){                
   if(!ereg("antani=",$querystring[$qs_nos]))
   {
...
$URL  =  array('scheme_host'  => $tmp[1],
               ...
               'completepath' => '/' . $tmp[6] . $tmp[7] . (isset($tmp[8]) ? $tmp[8] : '')
);
...
if ( preg_match('#^(?:127\.|192\.168\.|10\.|172\.(?:1[6-9]|2[0-9]|3[01])\.|localhost)#i', $URL['host']) ) {
   error('banned_site', false, $URL['host']);
}
...
if (// Option enabled (and possible? safe_mode prevents shell_exec)
...
if ( $fetch->abort ) {
   ...
   switch ( $fetch->abort ) {
      ...
      case 'http_status_error':
         ...
         error('http_error', $URL['completepath'], $fetch->status, trim(substr($fetch->headers[0], 12)), $explain);
         exit;
...
if ( $fetch->error ) {
   ...
   error('curl_error', $URL['completepath'], $fetch->error);
