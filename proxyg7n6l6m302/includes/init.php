...
function proxifyURL($url, $givenFlag = false) {
   ...
   global $CONFIG, $options, $bitfield, $flag, $hostname, $proxyurl;
   ...
   if ( $options['encodeURL'] ) {
      ...
      // Protect chars that have other meaning in URL
   ...
   if(preg_match("/".$hostname['onion']."/",$url)) {
      ...
      if(!$tmp){
        ...
        $new_onion = $tmp[2];
        return "http" . ( HTTPS ? 's' : '' ) . "://" . $tmp[2].$hostname['base'].$tmp[3];
      }
...
// Error message
function error($type, $toLoad = false) {
   ...
   // Remove first two argument (we have that as $type and $requestedUrl)
   array_shift($args);
   array_shift($args);
   ...
   redirect('index.php?e=' . $type . "&return=" . rawurlencode($toLoad) . $pass);
   exit;
