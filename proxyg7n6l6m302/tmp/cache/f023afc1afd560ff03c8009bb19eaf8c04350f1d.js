...
function submitDummyForm(theAction, theVariableName, theVariableValue, confirmMessage)
{
	...
	if (tmp)
	{
		...
		form.action = parseURL(theAction);
...
{
	document.write(' (<STRONG ID="' + idOfTrackerElement + '">' + numDefaultCharacters + '</STRONG> characters left)',"gl");
}
...
function createSnapbackLink(lastReplyId)
{
	...
	a.href = parseURL('#reply_' + lastReplyId);
